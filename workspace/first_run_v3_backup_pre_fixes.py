...
SUBSET_MAP = {
    ...
    "crown zenith":       ["crown zenith galarian gallery", "crown zenith: galarian gallery"],
    "celebrations 25th":  ["celebrations classic collection", "celebrations: classic collection"],
    "hidden fates":       ["hidden fates shiny vault", "hidden fates: shiny vault"],
    "shining fates":      ["shining fates shiny vault", "shining fates: shiny vault"],
}
...
def fetch_wizard_combined(set_name, wizard_entry, wizard_index, rate):
    ...
    for fragment in sub_set_fragments:
        # Find the sub-set in wizard index
        sub_entry = None
        for wname, entry in wizard_index.items():
            if fragment in wname or wname in fragment:
                sub_entry = entry
                break
        if sub_entry:
...
IMPORTANT: High Scarcity does NOT automatically mean a good investment. A set out of print for 10+ years has already been priced in by the market. Score L based on investment timing opportunity. Old sets like Team Up (2019), Cosmic Eclipse (2019), Unified Minds (2019) should score L=3 maximum as their price already fully reflects their OOP status.
