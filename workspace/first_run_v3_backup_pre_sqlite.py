...
ERA_MAP = {
    ...
    "black and white": "BW",
    "ex": "EX",
    "home": "MEGA",  # Perfect Order is Mega Evolution era
}

VALID_ERA_CODES = {"S&V", "SWSH", "MEGA", "SM", "XY", "BW", "EX"}

def normalise_era(raw_era):
    """Convert raw era string to short ALL CAPS code."""
    if not raw_era:
        return ""
    ...
    for k, v in ERA_MAP.items():
        if k in key:
            return v
    # Fallback: uppercase truncated
    return raw_era.strip().upper()[:6]

# ── Sub-set mapping ───────────────────────────────────────────────────────────
# Maps base set name (lower) -> list of sub-set name fragments to look for in wizard index
# Maps base set name (lower) -> exact wizard index name (lower) of sub-set
SUBSET_MAP = {
...
def get_fresh_token():
    """Get a fresh OAuth token for downloading files."""
...
def backfill_era(ws, sh, wizard_index):
    """Fix any Era values that are not in the correct short ALL CAPS format."""
    ...
    for i, row in enumerate(all_rows[1:], start=2):
        ...
        if needs_fix:
            ...
            if wizard_entry:
                ...
                # At least normalise what we have
...
def fetch_wizard_era(set_url):
    """Fetch era from a PokemonWizard set page. Returns (date_str, era_str)."""
    try:
    ...
    # Fallback: About paragraph "part of the X series"
    if not era:
...
def fetch_wizard_set_data(set_url, rate):
    """Fetch set page. Returns (total_usd, cards_list).
    cards_list = list of (name, usd_price) sorted by price desc."""
    try:
...
def fetch_wizard_combined(set_name, wizard_entry, wizard_index, rate):
    """
    Fetch set value and chase cards, combining base set + sub-sets.
    ...
    for fragment in sub_set_fragments:
        ...
        if sub_entry and sub_entry["url"] not in seen_sub_urls:
            ...
            if sub_total_usd:
                ...
                # Only add cards not already in base set (deduplicate by name)
    ...
    # Deduplicate cards by name, keep highest price version
    seen_names = {}
...
- Sets still IN PRINT must score L=1 or L=2 maximum. They cannot be scarce if they are still being printed.
- Sets out of print for 4+ years score L=3 maximum — their price already fully reflects OOP status, opportunity has passed.
- Only sets that went OOP within the last 2 years should score L=4 or L=5.
- High Scarcity does NOT automatically mean a good investment — an old OOP set has already been priced in by the market.
- Current in-print sets (2024-2026 releases still printing): Perfect Order, Ascended Heroes, Phantasmal Flames, Black Bolt, White Flare, Destined Rivals, Journey Together, Prismatic Evolutions, Surging Sparks, Stellar Crown, Twilight Masquerade, Temporal Forces, Paradox Rift, Obsidian Flames, Paldea Evolved, S&V Base Set — these must ALL score L=1 or L=2.

H recommendation must be exactly one of: Strong Buy, Buy, Accumulate, Hold, Reduce, Sell, Overvalued
...
def hex_to_rgb(hex_color):
    """Convert #RRGGBB to Google Sheets RGB dict (0.0-1.0)."""
    h = hex_color.lstrip('#')
    return {
...
def _cell_format(sheet_id, row_idx, col_idx, bg_hex, text_hex):
    """Apply background + text colour to a single cell."""
    return {
        "repeatCell": {
            "range": {
                "sheetId": sheet_id,
...
def apply_number_formats(ws, sh, num_rows):
    """Apply percentage format to col G, ratio format to col I."""
    try:
        sh.batch_update({"requests": [
            {
                "repeatCell": {
...
def apply_conditional_formatting(ws, sh, all_rows):
    """Apply light colour + coloured text to columns B, G, I, K only."""
    sheet_id = ws.id
    requests_batch = []
    ...
    for i, row in enumerate(all_rows[1:], start=2):
        ...
        # ── Column G — Box % (ratio stored, % displayed) ──────────────────
        try:
            g_val = float(col_val("box_pct").replace("%", ""))
            if g_val > 1.0:
                # Already stored as percentage number (e.g. 24.7) — normalise
                g_val = g_val / 100
            # g_val is now a ratio (0.247 etc.)
            if g_val > 1.0:
                bg, fg = "#FFCCCC", "#CC0000"   # red — bad
            elif g_val >= 0.75:
                bg, fg = "#FFE5CC", "#CC6600"   # orange — caution
            elif g_val >= 0.5:
                bg, fg = "#FFFACC", "#806600"   # yellow — neutral
            else:
                bg, fg = "#CCFFCC", "#006600"   # green — good
            requests_batch.append(_cell_format(sheet_id, i-1, COL["box_pct"]-1, bg, fg))
        except (ValueError, TypeError):
            pass

        # ── Column I — Chase Card % (ratio) ──────────────────────────────
        try:
            i_val = float(col_val("chase_pct").replace("%", ""))
            if i_val > 1.0:
                i_val = i_val / 100
            if i_val > 0.5:
                bg, fg = "#FFCCCC", "#CC0000"   # red — concentrated risk
            else:
                bg, fg = "#CCFFCC", "#006600"   # green — well spread
            requests_batch.append(_cell_format(sheet_id, i-1, COL["chase_pct"]-1, bg, fg))
        except (ValueError, TypeError):
            pass

        # ── Column K — Decision Score ─────────────────────────────────────
        try:
            k_val = int(float(col_val("decision")))
            if k_val >= 16:
                bg, fg = "#CCFFCC", "#006600"   # light green
            elif k_val >= 12:
                bg, fg = "#CCE5FF", "#003399"   # light blue
            elif k_val >= 8:
                bg, fg = "#FFFACC", "#806600"   # light yellow
            elif k_val >= 4:
                bg, fg = "#FFE5CC", "#CC6600"   # light orange
            else:
                bg, fg = "#FFCCCC", "#CC0000"   # light red
            requests_batch.append(_cell_format(sheet_id, i-1, COL["decision"]-1, bg, fg))
        except (ValueError, TypeError):
            pass

        # ── Column B — Date (age-based, respects Print Status) ───────────
        try:
...
def main():
    ...
    for i, row in enumerate(all_rows[1:], start=2):
        ...
        # Columns E & F — Set Value (combined) and Chase Cards
        wizard_entry = find_wizard_entry(set_name, wizard_index)
        set_value_gbp = None
        top3_usd_sum = None
        chase_str = None
        ...
        # Queue ALL sets for re-scoring with new rubric
        score_queue.append({
            "row_idx": i,
            "name": set_name,
    ...
    # Groq scoring — force re-score ALL sets with new rubric
