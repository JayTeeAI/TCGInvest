...
export function TrackerTable({ sets, user }: { sets: TCGSet[]; user: User | null }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/watchlist', { credentials: 'include' })
      .then(r => r.json())
      .then(data => {
        if (data.watchlist) setWatchlist(new Set(data.watchlist.map((i: {set_name: string}) => i.set_name)))
      })
      .catch(() => {})
  }, [isLoggedIn])
  ...
  async function handleWatchlist(e: React.MouseEvent, set: TCGSet) {
    ...
    if (inList) {
      ...
      if (res.ok) setWatchlist(prev => { const n = new Set(prev); n.delete(set.name); return n })
    } else {
        ...
        {filtered.map((set: TCGSet) => (
          <div key={set.id} className='bg-slate-900 border border-slate-800 rounded-xl p-4'>
            <div className='flex items-start justify-between gap-2 mb-3'>
              ...
              <span onClick={e => handleWatchlist(e, set)} className={'text-xl cursor-pointer shrink-0 ' + (watchlist.has(set.name) ? 'text-yellow-400' : 'text-slate-700')}>&#x2605;</span>
            </div>
            ...
            <div className='flex items-center justify-between mb-3'>
              <div>
                ...
                {isPremium ? (
                  ...
                  <a href={isLoggedIn ? '/premium' : '/auth/google'} className='text-xs text-yellow-500/60'>&#x1F512; \u2014</a>
                )}
              </div>
              <div className='text-right'>
                ...
                {isPremium ? (
                  ...
                  <a href={isLoggedIn ? '/premium' : '/auth/google'} className='text-xs text-yellow-500/50 whitespace-nowrap'>
                    &#x1F512; {isLoggedIn ? 'unlock \u2192' : 'sign in \u2192'}
                  </a>
                )}
              </div>
            </div>
        ...
        <div className='overflow-x-auto'>
          <table className='w-full'>
            <thead className='border-b border-slate-800'>
              <tr>
                <th className='px-3 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider w-8'>&#x2605;</th>
            ...
            <tbody className='divide-y divide-slate-800'>
              {filtered.map((set: TCGSet) => (
                <tr key={set.id} className='hover:bg-slate-800/60 cursor-pointer transition-colors' onClick={() => handleRowClick(set)}>
                  <td className='px-3 py-3' onClick={e => handleWatchlist(e, set)}>
                    <span className={'text-lg leading-none transition-colors ' + (watchlist.has(set.name) ? 'text-yellow-400' : 'text-slate-700 hover:text-slate-400')}>&#x2605;</span>
                  </td>
                  <td className='px-3 py-3'>
                    <div className='flex items-center gap-3'>
                      ...
                      <div>
                        <Link href={'/sets/' + setSlug(set.name)} className='font-medium text-white text-sm hover:text-blue-400 transition-colors' onClick={e => e.stopPropagation()}>
                          {set.name}
                        </Link>
                  ...
                  <td className='px-3 py-3 text-slate-200 text-sm font-medium'>{formatGBP(set.bb_price_gbp)}</td>
                  ...
                  <td className='px-3 py-3'>
                    {isPremium ? (
                      ...
                      <a href={isLoggedIn ? '/premium' : '/auth/google'} onClick={e => e.stopPropagation()}
                        className='flex items-center gap-1 group' title={isLoggedIn ? 'Upgrade to see AI scores' : 'Sign in free to get started'}>
                        <span className='text-xs font-bold px-2 py-1 rounded bg-yellow-500/10 text-yellow-500/60 border border-yellow-500/20 group-hover:border-yellow-500/50 transition-colors'>&#x1F512; \u2014</span>
                      </a>
                    )}
                  </td>
                  <td className='px-3 py-3'>
                    {isPremium ? (
                      ...
                      <a href={isLoggedIn ? '/premium' : '/auth/google'} onClick={e => e.stopPropagation()} className='group'>
                        <span className='text-xs text-yellow-500/50 group-hover:text-yellow-400 transition-colors whitespace-nowrap'>
                          &#x1F512; {isLoggedIn ? 'unlock why \u2192' : 'sign in \u2192'}
                        </span>
                      </a>
                    )}
                  </td>
